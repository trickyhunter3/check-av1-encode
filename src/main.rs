...
fn main() {
    ...
    //get clips
    let clip_names = extract_clips(&input_file, clip_length, clip_interval, &ffmpeg_path, &ffprobe_path).unwrap();
    ...
    let mut crf_values: Vec<i32> = Vec::new();
    //for each clip find crf
    for i in 0..clip_names.len() {
        let crf_90 = find_crf_for_90_ssim2(current_crf, 
            &clip_names[i], 
    ...
    let av1an_settings = format_encoding_settings(&av1an_setings_unformatted, &input_file, &speed, &crf_used_use.to_string(), &worker_num, &output_file);
...
fn spawn_a_process(args: [&str; 3], custom_error: &String) -> Result<i32, String>{
    ...
    if output.status.success(){
        ...
        return Err(custom_error.to_string() + &"no error, some program start and finished without doing anything".to_string());
    }
}
...
fn find_lowest_crf(crf_list: Vec<i32>) -> i32{
    let mut min = crf_list[0];
    for i in 1..crf_list.len(){
        if crf_list[i] < min{
            min = crf_list[i];
        }
    }
